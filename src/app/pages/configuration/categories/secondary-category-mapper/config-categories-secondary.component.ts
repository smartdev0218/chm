...
@Component({
  selector: 'chm-config-categories-secondary',
  templateUrl: './config-categories-secondary.component.html',
  styleUrls: ['./config-categories-secondary.component.css']
})
export class ConfigCategoriesSecondaryComponent implements OnInit {
  ...
  constructor(
    ...
    private configCategoriesService: ConfigCategoriesSecondaryService,
  ...
  adaptDataMappedCategories() {
    ...
    for(var i = 0; i<this.listMappedCategoriesAux.length; i++) {
      ...
      for(var j = 0; j<this.fListParentChannelCategories.length; j++) {
        ...
        if(catDestino.idCategoriaSecundariaCanal == mappedCategory.idCategoriaSecundariaCanal) {
...
//      }
  ...
  dropCategory($event: any, catDestino: any) {
      ...
      // Se guarda en BD
      this.configCategoriesService.mapCategory(this.idChannel, catDestino.idCategoriaSecundariaCanal, categoryDropped.id).subscribe(
          (data) => {
  ...
  disassociateCategory(siteCat: any, channelCat: any) {
      ...
      this.configCategoriesService.unmapCategory(this.idChannel, channelCat.idCategoriaSecundariaCanal, siteCat.id).subscribe(
        (data) => {
          console.log("Saved");
        },
        (error) => {
